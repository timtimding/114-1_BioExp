...
def preprocess_signal(data):
    ...
    #  (DC removal)
    # 1) robust DC：減去中位數（比平均值更抗尖峰）
    x = x - np.median(x)

    # 2) 慢速基線扣除：2 秒移動平均（可視情況改 1~5 秒）
    dc_window_sec = 2.0
...
class EnhancedBCIClassifier:
    ...
    def predict(self, X):
        ...
        # === student postprocessing === (calibration → smoothing → thresholding → majority vote)
        ...
        # Threshold fallback: use 0.5 if self.threshold_ is invalid
        ...
        if k_v > 1:
            if k_v % 2 == 0:  # Ensure an odd window to avoid ties
                k_v += 1
            pad = k_v // 2
            y_pad = np.pad(y_pred, (pad, pad), mode='edge')
            votes = np.convolve(y_pad, np.ones(k_v, dtype=int), mode='valid')
            y_pred = (votes > (k_v // 2)).astype(int)
